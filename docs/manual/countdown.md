...
AstraSchedule 提供两种倒计时：主窗口右侧边栏的基础倒计时，以及独立显示的倒数日窗口。两者共用管理端配置的倒数日数据。
...
每张卡片包含日程名称、剩余天数和描述性文字（如"距离中考还有 45 天"）。主卡片字号更大（天数 84px、名称 34px）。
...
| 天数范围 | 颜色效果 | RGB 值 |
|----------|----------|--------|
| ≥ 100 天 | 纯绿色 | `rgb(156, 255, 172)` |
| 50 至 99 天 | 绿色到黄色的线性渐变 | 逐日过渡 |
| 1 至 49 天 | 黄色到红色的线性渐变 | 逐日过渡 |
| 0 天（当天） | 纯红色 | `rgb(255, 104, 104)` |

颜色渐变通过 RGB 三通道的线性插值计算。展开模式下主副卡片的天数和名称均使用紧急性颜色渲染。
...
托盘菜单中的"课上计时""上课隐藏""始终缩小"三个开关会影响主窗口的迷你倒计时显示，但不影响独立倒数日窗口。倒数日窗口根据服务端数据运行，每 30 秒轮询一次更新数据。所有配置的倒数日均已过期时，窗口自动隐藏。
